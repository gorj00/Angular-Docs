...
- types of fetching the parameters (data): 
  ...
  - **Subsequent Changes**:
    - Updating changes of the data due to a route changes (loading the same component from the same component) can be achieved with an **observable** `route.params`.
 ```typescript
ngOnInit() {
    this.user = {
        // first initialization
    ...
    this.route.params
      .subscribe(
          // executes whenever the parameters changes from within (on the same component)
          (params: Params) => {
