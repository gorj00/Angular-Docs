...
- Route _navigate_ method is **unaware** what is its **current route (component)** (unlike _routerLinks_)
- therefore, by default, it **loads** the routes relative to the **route domain** `domain.com`,
...
- to do so, define a JavaScript object as a **second argument** of the _navigate_ Router method such as this: `{relativeTo: nameOfCurrentRoute}`:
```typescript
onButtonClick() {
    this.router.navigate(['users', 'user', 'Jack'],                                     {relativeTo: currentRoute});
}