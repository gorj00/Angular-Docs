...
- loading a router by some action or operation being executed (method and so on). 

**1. step**
- In **.html file**, set for example a button - after clicking on it, a **method is executed**:
```html
...
- define your custom method (onButtonClick() in this example) and navigate to a route using the injected router:
```typescript
onButtonClick() {
    this.router.navigate(['users', 'user', 'Jack']);
}
```
- **navigate** method of a router takes an argument of an **array** (items of the array are parts of the router path).
