...
- It is possible to create routes with **dynamic segments** in your path OF the route.
...
**3. step**
- It depends whether we want to load a routed component and its data only once, or whether we want to be able to detect changes of data on the component,
- for example: _you use a user component that contains the data of the user, the concrete user data is determined by a dynamic router, therefore, you use one component that can display many variations based on the queried user - the data needs to be allowed to be updated when going from an already loaded component's variation (for instance from user A) to another variation (user B)_,
...
- changes on the same component will **not update** the changed data. 
**Subsequent Changes**
- This can be achieved with **observable** `route.params` 
