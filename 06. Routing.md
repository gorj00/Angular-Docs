...
- If one page has several areas to route different components (uses several router **secondary** outlets), **name** the router outlets to be able to **spot** the specific router outlet by the **defined route**, 
- if router outlet is not named, it is considered to be **primary**, such an outlet will match routes without an **outlet** property definition.
...
**2. step**
- In **.ts router definitions file**, **specify** which _router outlet_ to use to route the child component: 
```typescript
const appRoutes: Routes = [
    { path: 'parent', component: ParentComponent, children: [
        ...
