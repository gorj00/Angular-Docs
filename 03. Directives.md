...
- the object contains **key value pairs** of _class name as the key_ and _condition allowing/disallowing attaching the class as the value_:
  - **class name** must be written in:
    ...
    - camel case notaion **doesn't work** as it does in _ngStyle_,
  ...
  - **multiple classes** are separated by a _comma (,)_ `{class1: expression1, class2: expression2}`,
    - multiple classes depending on **the same condition** may be simplified: they are not separated by a comma and are together in **single quotes**: `{'class1 class2': expression}`:
```html
