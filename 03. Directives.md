...
- the property must **receive a value** as parameter (of boolean type):
```typescript
...
**3. step** 
Structural directive (used with a star `*`) is then transformed into an attribute directive of **ng-template** tag, therefore, you need to get access to the **template** and the **place in the document** where the conditioned content is supposed to be rendered. Both can be **injected**. 
...
export class UnlessDirective {
  @Input() set appUnless(condition: boolean) {
    ...
    if (!condition) {
      // ... include the element from the DOM
...
<div *appUnless="condition">
  <p>I show up only if the condition is false.</p>
</div>
```